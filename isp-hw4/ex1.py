...
def anonymize_2(db):
    ...
    anon = db[:]
    for entry in anon:
        salted_name = entry[0] + salt
        digest = hashlib.sha512(salted_name.encode()).hexdigest()
        entry[0] = digest
        entry[2] = '*'
    return anon
...
def get_top_rated(anon, movie):
    users_rated_this_movie = []
    for entry in anon:
        if entry[1] == movie:
            users_rated_this_movie.append(entry[0])

    results = []
    for user in users_rated_this_movie:
        rated_movies = {1:[], 2:[], 3:[], 4:[], 5:[]}
        for entry in anon:
            if user == entry[0]:
                rated_movies[entry[3]].append(entry[1])
        for i in range(5, 0, -1):
            if len(rated_movies[i]) != 0:
                results = results + rated_movies[i]
                break
